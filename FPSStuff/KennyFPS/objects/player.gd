...
var container_offset = Vector3(1.2, -1.1, -2.0)
...
func action_shoot():
	...
	if Input.is_action_pressed("shoot"):
		...
		# Shoot a single projectile orb (replaces hitscan multi-shot)
		var proj_scene: PackedScene = preload("res://objects/projectiles/basic_rigid_body_projectile.tscn")
		var proj_mgr = proj_scene.instantiate()
		get_tree().current_scene.add_child(proj_mgr)
		# Spawn slightly in front of the camera (approximate muzzle)
		var origin: Vector3 = camera.global_transform.origin
		var forward: Vector3 = -camera.global_transform.basis.z
		var right: Vector3 = camera.global_transform.basis.x
		var up: Vector3 = camera.global_transform.basis.y
		var spawn_origin: Vector3 = origin + forward * 0.6 + right * 0.1 + up * -0.1
		proj_mgr._Set_Projectile(int(weapon.damage), Vector2.ZERO, int(weapon.max_distance), spawn_origin)
		
		# Weapon recoil feedback
